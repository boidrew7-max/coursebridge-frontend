...
def _build_profile_context(user_profile):
    ...
    # CourseBridge planner data
    completed = user_profile.get("completed") or []
    missing   = user_profile.get("missing") or []
    blocked   = user_profile.get("blocked") or []
    readiness = user_profile.get("readiness_score")

    if completed:
        names = ", ".join(c.get("code", c) if isinstance(c, dict) else str(c) for c in completed)
        parts.append(f"Completed Courses: {names}")
    if missing:
        names = ", ".join(c.get("code", c) if isinstance(c, dict) else str(c) for c in missing)
        parts.append(f"Missing Requirements: {names}")
    if blocked:
        names = ", ".join(c.get("code", c) if isinstance(c, dict) else str(c) for c in blocked)
        parts.append(f"Blocked Courses (prerequisites not met): {names}")
    if readiness is not None:
        parts.append(f"Transfer Readiness Score: {readiness}%")

    if not parts:
        return ""
    lines = "\n".join(f"- {p}" for p in parts)
    return (
        "\n\n=== THIS STUDENT'S TRANSFER PLAN (from CourseBridge) ===\n"
        + lines
        + "\nPersonalize every answer to their specific situation. Reference their completed and missing courses directly when relevant."
        + "\n=== END PLAN ==="
    )
