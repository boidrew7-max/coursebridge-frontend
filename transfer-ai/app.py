...
# Allow requests from CourseBridge AI (Next.js) frontend
ALLOWED_ORIGIN = os.getenv("COURSEBRIDGE_ORIGIN", "http://localhost:3000")

@app.after_request
def cors(response):
    origin = request.headers.get("Origin", "")
    if origin == ALLOWED_ORIGIN or os.getenv("FLASK_ENV") == "development":
...
def _build_planner_profile(data):
    """Build a user_profile dict from CourseBridge planner context."""
    planner = data.get("plannerContext") or {}
    return {
        "college":        planner.get("college", ""),
        "major":          planner.get("major", ""),
        "target_schools": planner.get("targetUniversity", ""),
        "completed":      planner.get("completedCourses", []),
        "missing":        planner.get("missingCourses", []),
        "blocked":        planner.get("blockedCourses", []),
        "readiness_score": planner.get("readinessScore"),
    }
...
def chat():
    ...
    if not _check_rate(ip):
        ...
        def rate_msg():
            msg = f"You've sent {RATE_LIMIT} messages this hour. Please wait about {minutes} minute{'s' if minutes != 1 else ''} and try again."
    ...
    # Build profile from either CourseBridge planner context or logged-in user
    uid = session.get("user_id")
    if data.get("plannerContext"):
        user_profile = _build_planner_profile(data)
    elif uid:
...
def api_profile_get():
    ...
    if not uid:
        return jsonify({"error": "Not authenticated"}), 401
    user = get_user_by_id(uid)
    return jsonify(_public(user)) if user else (jsonify({"error": "Not authenticated"}), 401)
...
def api_profile_put():
    ...
    if not uid:
        return jsonify({"error": "Not authenticated"}), 401
    data   = request.json or {}
    fields = {k: data[k] for k in ("username", "college", "major", "target_schools", "onboarded") if k in data}
...
def api_sessions_create():
    ...
    return jsonify(create_session(uid, title)), 201
...
def api_messages_get(sid):
    ...
    return jsonify(msgs) if msgs is not None else (jsonify({"error": "Not found"}), 404)
...
def api_messages_post(sid):
    ...
    return ("", 204) if add_messages(sid, uid, msgs) else (jsonify({"error": "Not found"}), 404)
